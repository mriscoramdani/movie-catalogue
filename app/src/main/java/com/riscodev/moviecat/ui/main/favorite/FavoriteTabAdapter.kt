...
class FavoriteTabAdapter(private val mContext: Context, fm: FragmentManager) :
    ...
    override fun getItem(position: Int): Fragment =
        when (position) {
            0 -> MovieFragment.newInstance(MovieFragment.MENU_FAVORITE)
            1 -> ShowFragment.newInstance(ShowFragment.MENU_FAVORITE)
            else -> Fragment()
        }
